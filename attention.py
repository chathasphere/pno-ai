...
class MultiheadedAttention(nn.Module):
    ...
    def __init__(self, d_model, heads=8, dropout=0.1):
        ...
        self.unifyheads = nn.Linear(heads * s, d_model)
    ...
    def forward(self, x, mask):
        ...
        #Compatibility scores of queries and keys via dot product
        scores = torch.bmm(queries, keys.transpose(1, 2))

        # mask out the upper half of the dot matrix, excluding the diagonal
        if mask is not None:
            #repeat mask per head...
            mask = torch.repeat_interleave(mask, h, 0)
